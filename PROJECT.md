...
После TASK-013 к документальному фундаменту, каркасу, границе пакета
...
В текущий объём по-прежнему не входят path-level orchestration полного потока,
запись JSON на диск, пользовательский CLI,
