...
После TASK-012 к документальному фундаменту, каркасу, границе пакета
...
provenance без Pydantic, JSON и файлов. Первый срез
по-прежнему различает публикацию источника и будущий физический объект.

В текущий объём по-прежнему не входят Pydantic-модель результата и её
валидация, JSON-сериализация, пользовательский CLI,
