...
После TASK-010 к документальному фундаменту, каркасу, границе пакета
...
`search-criteria@1`. Она атомарно преобразует один Path/UTF-8/JSON документ
в полный neutral immutable `SearchCriteria` из канонических `Money`, `Area` и
`RoomCount` либо в упорядоченные `ContractIssue`. Первый срез по-прежнему
различает публикацию источника и будущий физический объект.

В текущий объём по-прежнему не входят поиск, `SearchMatch`/`SearchResult`,
output mapping, Pydantic-модель результата, JSON-сериализация, пользовательский CLI,
