...
После TASK-006 существуют переносимый документальный фундамент, выбранный минимальный технологический базис, проект первого локального сквозного среза, спецификация его предметной модели, воспроизводимая стратегия проверок с полностью вымышленными JSON-фикстурами и байтовыми эталонами, минимальный устанавливаемый Python-каркас с единой командой качества и строгая граничная загрузка одного локального пакета `fixture-source-batch@1` в неизменяемый типизированный результат. Первый срез различает публикацию источника и будущий физический объект, хранит происхождение у каждого нормализованного значения и явно представляет отсутствие и неподдерживаемое значение.

В текущий объём по-прежнему не входят предметные модели ядра, Pydantic-модели критериев и результата, source adapter, нормализация, поиск, пользовательский CLI, база данных, API, интерфейс, парсеры конкретных площадок, Docker, ИИ, OpenClaw, Telegram и публикация удалённого репозитория. Существующий Python-код ограничен package foundation, quality runner, внешней boundary-моделью пакета публикаций, нейтральным предадаптерным контрактом и их тестами. Выбор Python-базиса не является выбором всех будущих инфраструктурных компонентов: они добавляются только по подтверждённой потребности. Соблюдение правовых требований, условий площадок, ограничений на сбор и хранение данных будет конкретизировано до реализации реальных источников.
