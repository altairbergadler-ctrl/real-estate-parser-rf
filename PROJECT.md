...
После TASK-008 существуют переносимый документальный фундамент, выбранный минимальный технологический базис, проект первого локального сквозного среза, спецификация его предметной модели, воспроизводимая стратегия проверок с полностью вымышленными JSON-фикстурами и байтовыми эталонами, минимальный устанавливаемый Python-каркас с единой командой качества, строгая граничная загрузка одного локального пакета `fixture-source-batch@1`, чистый фиктивный source adapter и чистая нормализация одного `SourcePublicationSnapshot`. Нормализатор точно преобразует время, location, деньги, валюту, площадь и комнаты, хранит полное происхождение каждого значения/состояния и атомарно отказывает без partial listing. Первый срез по-прежнему различает публикацию источника и будущий физический объект.

В текущий объём по-прежнему не входят пакетная нормализация, Pydantic-модели критериев и результата, коллекция, поиск, пользовательский CLI, база данных, API, интерфейс, парсеры реальных площадок, Docker, ИИ, OpenClaw, Telegram и публикация удалённого репозитория. Существующий Python-код ограничен package foundation, quality runner, внешней boundary-моделью пакета публикаций, нейтральными контрактами до и после адаптации, одним фиктивным адаптером, нормализованными типами и операцией одного снимка и их тестами. Выбор Python-базиса не является выбором всех будущих инфраструктурных компонентов: они добавляются только по подтверждённой потребности. Соблюдение правовых требований, условий площадок, ограничений на сбор и хранение данных будет конкретизировано до реализации реальных источников.
