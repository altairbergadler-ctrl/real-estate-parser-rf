...
TASK-015 принимает следующий предметный дизайн, не изменяя исполняемый срез:
несколько наблюдений объединяются только внутри потока одной `PublicationRef`,
а последовательные immutable observations сравниваются версионированной
детерминированной policy. Подтверждённая недоступность является отдельным
доказательным наблюдением; отсутствие публикации в batch, timeout или source
failure не доказывают исчезновение. Reappearance не создаёт физический объект
и не стирает provenance предыдущих наблюдений. Python-типы, хранение и
интерфейсы этой модели ещё не реализованы.

В текущий объём по-прежнему не входят запись JSON на диск, реализация истории
наблюдений, база данных, API,
