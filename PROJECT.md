...
После TASK-009 существуют переносимый документальный фундамент, выбранный минимальный технологический базис, проект первого локального сквозного среза, спецификация его предметной модели, воспроизводимая стратегия проверок с полностью вымышленными JSON-фикстурами и байтовыми эталонами, минимальный устанавливаемый Python-каркас с единой командой качества, строгая граничная загрузка одного локального пакета `fixture-source-batch@1`, чистый фиктивный source adapter, поштучная нормализация и атомарная операция полного `SourceBatch`. Она собирает все independently provable normalization issues и только при полном успехе создаёт immutable `CollectionSnapshot` с уникальными `PublicationRef` и сохранённым входным порядком. Первый срез по-прежнему различает публикацию источника и будущий физический объект.

В текущий объём по-прежнему не входят Pydantic-модели критериев и результата, поиск, output mapping, пользовательский CLI, база данных, API, интерфейс, парсеры реальных площадок, Docker, ИИ, OpenClaw, Telegram и публикация удалённого репозитория. Существующий Python-код ограничен package foundation, quality runner, внешней boundary-моделью пакета публикаций, нейтральными контрактами до и после адаптации, одним фиктивным адаптером, нормализованными типами, операцией одного снимка, атомарным batch-to-collection переходом и их тестами. Выбор Python-базиса не является выбором всех будущих инфраструктурных компонентов: они добавляются только по подтверждённой потребности. Соблюдение правовых требований, условий площадок, ограничений на сбор и хранение данных будет конкретизировано до реализации реальных источников.
