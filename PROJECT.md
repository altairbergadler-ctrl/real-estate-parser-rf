...
После TASK-005 существуют переносимый документальный фундамент, выбранный минимальный технологический базис, проект первого локального сквозного среза, спецификация его предметной модели, воспроизводимая стратегия проверок с полностью вымышленными JSON-фикстурами и байтовыми эталонами, а также минимальный устанавливаемый Python-каркас с единой командой качества. Первый срез различает публикацию источника и будущий физический объект, хранит происхождение у каждого нормализованного значения и явно представляет отсутствие и неподдерживаемое значение.

В текущий объём по-прежнему не входят предметные модели, Pydantic-модели внешних документов, source adapter, нормализация, поиск, пользовательский CLI, база данных, API, интерфейс, парсеры конкретных площадок, Docker, ИИ, OpenClaw, Telegram и публикация удалённого репозитория. Существующий Python-код ограничен техническим package foundation, quality runner и инфраструктурными тестами. Выбор Python-базиса не является выбором всех будущих инфраструктурных компонентов: они добавляются только по подтверждённой потребности. Соблюдение правовых требований, условий площадок, ограничений на сбор и хранение данных будет конкретизировано до реализации реальных источников.
