...
- **TASK-008 — детерминированная нормализация одного снимка.** Нормализовать один `SourcePublicationSnapshot` в `NormalizedListing` с точными преобразованиями, происхождением и состояниями `Present`/`Missing`/`Unsupported`, без коллекции и поиска. Рекомендуемая следующая задача.
