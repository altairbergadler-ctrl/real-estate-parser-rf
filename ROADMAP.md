...
- **TASK-014 — CLI и итоговый сквозной тест первого среза.** Связать уже готовые границы в один path-level поток, добавить пользовательскую команду и проверить stdout/stderr, exit codes, атомарность и golden bytes. Завершено в task-ветке; первый локальный сквозной срез завершён и готов к review/merge.

Каждый пункт перед началом оформляется отдельной задачей; реализация этапа не объединяется в одну крупную ветку.

После интеграции TASK-014 следующий безопасный шаг — отдельный пользовательский
выбор одного пункта этапа 3 и его декомпозиция в новую малую задачу. TASK-015
заранее не определяется, а реализация этапа 3 автоматически не начинается.
