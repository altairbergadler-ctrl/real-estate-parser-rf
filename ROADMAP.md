...
- **TASK-006 — граничная загрузка локального пакета публикаций v1.** Строгая Pydantic-граница `fixture-source-batch@1`, чтение локального JSON и явно типизированный выход для будущего source adapter без адаптации, нормализации или поиска. Рекомендуемая следующая задача.
- Адаптировать проверенный пакет через контракт фиктивного источника.
