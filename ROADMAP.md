...
- **TASK-009 — атомарная нормализация пакета и immutable collection.** Нормализовать все snapshots полного `SourceBatch`, собрать ошибки и только при полном успехе построить `CollectionSnapshot` с уникальными `PublicationRef`, без поиска и output mapping. Рекомендуемая следующая задача.
