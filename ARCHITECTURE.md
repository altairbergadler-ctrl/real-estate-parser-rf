...
Модульный монолит — архитектурное направление, а не выбор языка, фреймворка, базы данных или способа развёртывания. Эти решения в TASK-001 не принимаются.

## Минимальное стабильное ядро

Ядро содержит только устойчивые предметные понятия и контракты, необходимые нескольким модулям: идентичность объявления и объекта, происхождение данных, нормализованные значения, критерии поиска, доказательства, сигналы и результаты обработки.
...
Точный состав типов ядра будет определён отдельной задачей после выбора минимального сквозного среза.
...
- **Устранение дублей** определяет возможные публикации одного объекта, хранит основания сопоставления и не уничтожает происхождение отдельных объявлений.
...
Хранилище, планирование запусков, наблюдаемость и пользовательские интерфейсы являются инфраструктурными или внешними аспектами. Их границы уточняются только тогда, когда этого требует выбранный сквозной срез.
