...
Фикстуры полностью вымышлены, не описывают реальные объявления и не требуют
сети. Документ задаёт тестовые данные и ожидаемое поведение; фактические
Python/Pydantic/pytest/CLI реализации находятся в коде соответствующих задач и
не меняют этот oracle.
...
утверждённых search golden через offline fixture pipeline. TASK-014 покрывает
application composition отдельно от CLI и subprocess-интеграцию полного среза:
...
Единая команда качества — `uv run quality`; она в установленном порядке
запускает Ruff format-check, Ruff lint, строгий mypy, pytest, проверку manifest
и байтовых golden-файлов. Каталог дополнительно подтверждает, что стандартная
библиотека разбирает все `.json`, кроме явно зарегистрированного `SYN-001`, а
он обязан завершиться синтаксической ошибкой.
