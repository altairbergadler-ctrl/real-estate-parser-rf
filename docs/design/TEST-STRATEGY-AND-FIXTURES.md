...
| `rooms` | string | необязательно | `studio` или ASCII-десятичное целое без знака |
...
- `rooms = studio` даёт `0`; десятичная строка даёт соответствующее целое `0..99`. Иные значения — `invalid_value` или `out_of_range`.
