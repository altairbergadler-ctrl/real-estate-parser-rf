...
Сейчас в репозитории находится документальный фундамент, проект первого локального сквозного среза, спецификация его предметной модели, полностью вымышленные JSON-фикстуры, минимальный устанавливаемый Python-каркас, строгая граница `fixture-source-batch@1`, чистый фиктивный source adapter, детерминированная нормализация, атомарная immutable-коллекция и строгая граница `search-criteria@1` с каноническим `SearchCriteria`. Сам поиск, выходное представление, пользовательский CLI, база данных, API, интерфейс, парсеры реальных площадок, ИИ и интеграции ещё не реализованы.
