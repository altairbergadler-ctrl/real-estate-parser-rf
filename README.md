...
Сейчас в репозитории находится документальный фундамент, проект первого локального сквозного среза, спецификация его предметной модели, полностью вымышленные JSON-фикстуры, минимальный устанавливаемый Python-каркас, строгая библиотечная граница загрузки одного локального пакета `fixture-source-batch@1`, чистый фиктивный source adapter, поштучная детерминированная нормализация с полным provenance и атомарная сборка immutable `CollectionSnapshot` с проверкой уникальности `PublicationRef`. Граница criteria, поиск, выходное представление, пользовательский CLI, база данных, API, интерфейс, парсеры реальных площадок, ИИ и интеграции ещё не реализованы.
